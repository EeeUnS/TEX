...
% \end{thm}
\begin{document}
    \begin{thm}[Graph] 정점과 정점을 잇는 간선들로 이루어진 것을 그래프라한다.
        \begin{itemize}
            \item $V(G)$ :  정점의 집합
            \item $E(G)$ :  간선의 집합
    ...
    \begin{thm}[e special classes of graphs]
        \begin{itemize}
...
$\dfrac{v(v-1)}{2}$가 최댓값이자 그래프의 간선수 + 여그래프의 간선수 입니다. 그래프의 간선수 = 여그래프의 간선수 이므로 $v$나 $v-1$은 적어도 둘 중 하나는(적어도지만 사실 둘다 4의 배수인 경우의 수는 존재하지않습니다) 4의 배수여야합니다
    ...
    \begin{thm}[degree(차수)] $d_G(v)$는 정점 $v$에 연결된 간선의 갯수를 나타낸다. 그래프의 정점의 차수의 최솟값을 $\delta(G)$ , 최댓값을 $\Delta(G)$로 표기한다.
    ...
    If G has vertices VI, V2, .•. , v"' the sequence (d(Vl), d(V2)' ... , d(v,,)
is called a degree sequence of G. Show that a sequence
(d t , d2 , ••• , do) of non-negative integers is a degree sequence of some n
...
???
    ...
    \begin{thm}[walk]순차적으로 이어지는 정점, 간선의 연결을 walk라한다. $v_{0}e_{1}v_{1}e_{2}v_{2} \cdots e_{k}v_{k}$인 walk를 $v_0$ to $v_k$ 또는 ($v_0$, $v_k$)-walk라 한다.
        \begin{itemize}
...
\item 1.8.6
\begin{thm}[tree] connected acyclic graph
...
week 4
\item  2.1.5
Let 0 be a graph with v-I edges.
 Show that the following three
...
나무 하나가 정확히 하나의 중심 또는 두 개의 인접한 중심을 가지고 있음을 보여줍니다.
\end{itemize}
\end{document}
